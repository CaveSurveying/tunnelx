...
System.out.println("stat " + ixs0);
	...
	{
		TN.emitMessage("Copying centreline image at angle: " + angdeg + " with scale " + scalefac);
		...
		// we make a new endpoint for every single piece,
		// and connect with connective pieces to an origin

		OnePathNode pnorigin = new OnePathNode(0.0F, 0.0F, 0.0F, true);
...
//					pnend = (OnePathNode)tsketch.vnodes.elementAt(op.pnend.pathcountch);
				...
				nop.linestyle = SketchLineStyle.SLS_DETAIL;  // will need some labels to help say it should be drawn.  Any other type forms areas
				nop.EndPath(pnend);

				nop.vssubsets.add("elevcopy");
				nop.vssubsets.addAll(op.vssubsets);
				for (int j = 0; j < op.vssubsets.size(); j++)
                	nop.vssubsets.add(((String)op.vssubsets.elementAt(j)) + "-elev");

				nop.importfromname = "elevcopy";
				AddPath(nop);

				// the add path adds in the nodes, and we have to get their cross indexes
				// (this is not useful unless we are connecting things properly
				if (op.pnstart.pathcountch == -1)
					op.pnstart.pathcountch = tsketch.vnodes.indexOf(pnstart);
				if (op.pnend.pathcountch == -1)
					op.pnend.pathcountch = tsketch.vnodes.indexOf(pnend);

				// put in the connective path
				OnePath cnop = new OnePath(pnorigin);
				cnop.linestyle = SketchLineStyle.SLS_CONNECTIVE;  // will need some labels to help say it should be drawn.  Any other type forms areas
				cnop.EndPath(pnstart);

				cnop.vssubsets.add("elevcopy");
				cnop.importfromname = "elevcopy";
				AddPath(cnop);
			}
		}
