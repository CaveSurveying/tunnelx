...
class OneSection 
{
	...
	/////////////////////////////////////////////
	// read section
	OneSection(LineInputStream lis) throws IOException
	{
		lis.FetchNextLine(); 
		...
		lambda = Float.valueOf(lis.w[3]).floatValue(); 

		lis.FetchNextLine(); 
		orientstationforeS = lis.w[0]; 
		orientstationbackS = lis.w[1]; 
		relorientcompassS = lis.w[2]; 
		orientclinoS = lis.w[3]; 

		lis.FetchNextLine(); 
		if (!lis.w[0].equalsIgnoreCase("No_nodes")) 
			System.out.println("Error: No_nodes missing"); 
		int lnnodes = Integer.valueOf(lis.w[1]).intValue(); 
		for (int i = 0; i < lnnodes; i++)
		{
			lis.FetchNextLine(); 
			AddNode(new Vec3(lis.w[0], lis.w[1], lis.w[2])); 
		}
	...
	public boolean ReformRspace()
	{
		...
		// set up the matrix
		rspace.unit(); 
		rspace.xrot(orientclino * TN.angfac - Math.PI / 2); 
		rspace.zrot(-orientcompass * TN.angfac); 
	...
	void LoadFromShapeGraphics(ShapeGraphics shp)
	{
		...
		boolean bClockwise; 
		{
			...
			if (!bClockwise) 
				System.out.println("Dereflecting XSection!!!"); 
		}
