...
class SectionDisplay extends JFrame
{
	...
	void LoadPrevXS()
	{
		...
		{
			TN.currentDirectory = sfd.getCurrentDirectory(); 
			sectionpreviewdisplay.LoadPrevSections(sfd.svxfile); 
		}
	}
	...
	void SavePrevXS()
	{
		...
		{
			TN.currentDirectory = sfd.getCurrentDirectory(); 
			sectionpreviewdisplay.SavePrevSections(sfd.svxfile); 
		}
	}
...
setTitle("Section for " + xsection.station0S + " " + String.valueOf(xsection.lambda)); 
	...
	boolean SectionApply()
	{
		...
		{
			System.out.println("bad lambda/relorient compass/clino"); 
			return(false); 
		}
		...
		{
			System.out.println("station 0 or 1 not found"); 
			return(false); 
		}
		...
		if (bNewSection && lstation0S.equalsIgnoreCase(lstation1S)) 
		{
			for (int i = 0; i < vsections.size(); i++) 
			{
				...
				{
					System.out.println("xsection on station already present"); 
					return(false); 
				}
			}
		}
		...
		if ((lorientstationforeS.length() != 0) && (lstationfore == null))  
			System.out.println("Warning, fore station not found: " + lorientstationforeS); 
		if ((lorientstationbackS.length() != 0) && (lstationback == null))  
			System.out.println("Warning, back station not found: " + lorientstationbackS); 
